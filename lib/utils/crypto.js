...
const ENCRYPTO_METHOD = 'aes-256-gcm'
const PASSWORD = 'interview'
const KEY = crypto.scryptSync(PASSWORD, 'saltttttt', 32)

const RESET = Buffer.alloc(0)

const _encrypt = (data, _nonce = null) => {
    let nonce = _nonce || crypto.randomBytes(16)
    let cipher = crypto.createCipheriv(ENCRYPTO_METHOD, KEY, nonce)
...
const _decrypt = (data, tag, nonce) => {
    try {
        let decipher = crypto.createDecipheriv(ENCRYPTO_METHOD, KEY, nonce)
